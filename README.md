...
Jeved is a SillyTavern extension. It uses Jev, a small and cheap decision model, to read each reply
in your chat and answer questions that you write. A rule on the answers can add one line to your
next prompt, reroll the reply, or run an STscript.
...
from your list, or the chance that a statement is true. The built-in preset covers pacing, tone and
repeated wording. You can change it, write your own, and share a preset as one file.
...
A rule fires when its conditions match on N of the last M replies. A condition tests one sensor.
Score and Noul use "below" or "above" a value. Choice uses "is" or "is not" an option. A Score or
Choice condition can also require a minimum confidence. If the host sends no confidence for a reply,
that condition does not match on that reply.
...
disaster right now." The Change sensor asks how much the situation changes. The Flat rule fires when
3 of the last 4 replies have Change below 2.5 and Tension below 1.5. It then adds this line to your
...
with the first rule highest. The nudge spacing of the preset is the smallest number of replies
between two nudges.

There are two actions.

- Nudge. Jeved adds the instruction to the copy of your next message that goes into the prompt. It
  does not change your message in the chat, so old instructions do not collect in the context.
- Reroll. Jeved swipes the reply one time and adds the instruction to that generation. Jeved rerolls
  ...
  start to type, edit the reply, swipe, or change chat. Rerolls do not run in group chats.
...
Sensors that have the same History, "Include my messages" and "Include context" settings share one
API call. The built-in preset makes about two calls for each reply.
...
chat" blocks all of them in that chat. Each call goes to the endpoint you set, and it contains only:

- the last replies that the sensor reads, and your messages when "Include my messages" is on
- the character card, your persona and the prompts that you ticked under "Context sent to Jev" in
  Settings, when the sensor has "Include context" on. Preview there shows the exact text.
...
A preset contains the sensors, the rules, the context checklist and the nudge limits. The preset
buttons are on the Settings tab.
...
The Picture rule in the built-in preset is an example. It is off, and it needs an image backend.

```
/imagine quiet=true scene
```

Without `quiet=true`, `/imagine` posts the picture into the chat in the middle of your turn.
...
- `/jeved-rescan count=<n>` measures the recent replies that have no answer. It asks first.
...
The answers and decisions that Jeved wrote on your messages stay in the chat files. "Clear scores"
