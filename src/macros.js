...
import { NOUL, findSensor, isKnownType, macroText, typeIds } from './sensor-types.js';
...
export function answerText(sensorId) {
    ...
    return macroText(sensor, latestScores(chat, [sensor.id], chat.length, preset.sensors)[sensor.id]);
}

let hostMacro = false;

export function fillMacros(text) {
    return hostMacro ? text : String(text ?? '').replace(REFERENCE, (whole, id) => answerText(id));
}
...
export function initMacros() {
    ...
    macros.register(MACRO_NAME, {
        ...
        unnamedArgs: [{ name: 'sensor', description: 'The id of the sensor to read.' }],
        ...
        exampleUsage: ['{{jeved::scene}}'],
        handler: ({ unnamedArgs }) => answerText(unnamedArgs?.[0]),
    });
    return true;
}
...
export function askSensor(args = {}, question = '') {
    ...
    return {
        ...
        context: TRUE_WORDS.includes(String(args.context ?? '').trim().toLowerCase()),
